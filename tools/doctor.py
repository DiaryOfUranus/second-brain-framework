...
检测与自愈项：
  ...
  2. Git 完整性       仓库 .git 是否丢失/损坏（丢失给一键重建配方，不自动跑，需你确认远程）
  3. 远程连通性       github.com:443 与 api.github.com；443 不可达自动指出 API 通道兜底
  4. 脑状态文件       index.md / ledger.md / VERSION.json 缺失 → 从 templates/ 生成（--fix）
  5. 健康检查钩子     .git/hooks/post-commit 是否就位（--fix 自动装）
  6. SKILL.md 引用     references/ docs/ 等相对链接是否都真实存在（断裂即指出）
  7. 脱敏残留         跑 scrub --check，发现残留给下一步命令
...
REPO = os.path.dirname(HERE)          # 仓库/脑根
...
def check_git_integrity(results, do_fix):
    git_dir = os.path.join(REPO, ".git")
    if os.path.isdir(git_dir):
        results.append(("ok", "Git 完整性", ".git 存在，仓库完整"))
        return
    # .git 丢失/损坏 —— 不自动重建（需你的远程 URL，且涉及重写历史）
    results.append(("red", "Git 完整性",
                    ".git 不存在或已损坏：仓库失去版本控制（工作区文件仍在，数据未丢）"))
    recipe = (
        "cd <仓库根> && "
        "git init -b main && "
...
def check_brain_state(results, do_fix):
    ...
    for f in missing:
        ...
        if do_fix and os.path.exists(tpl):
            try:
                import shutil
                shutil.copy(tpl, os.path.join(REPO, f))
                results.append(("fix", f"补全 {f}", f"已从 templates/{f} 生成 ✓"))
                continue
            except Exception as e:
                results.append(("red", f"补全 {f}", f"复制失败：{e}"))
        elif os.path.exists(tpl):
            results.append(("yellow", f"缺失 {f}",
                            f"可从模板生成：cp templates/{f} ./{f}"))
        else:
            results.append(("yellow", f"缺失 {f}",
                            "无对应模板；请按 SKILL.md 的脑文件规范手动创建"))


def check_hook(results, do_fix):
...
def main():
    ...
    check_git_integrity(results, args.fix)
    check_connectivity(results)
    check_brain_state(results, args.fix)
    check_hook(results, args.fix)
